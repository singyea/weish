...
{
	${CURL} ${WEIBO_LOGIN_PRE_PAGE} 2>/dev/null >sina_login_pre_page.html
...
function login_put_user_pass()
{
	...
	${CURL}  -D login_state_header.txt --data ${WEIBO_LOGIN_POST_DATA} ${WEIBO_LOGIN_SUBMIT_PAGE}"?rand="${WEIBO_PRE_LOGIN_RAND}"&backURL=http://weibo.cn/dpool/ttt/home.php&backTitle=新浪微博&vt=4&ns=1" > /dev/null 2>&1
	LOGIN_STATE=`cat login_state_header.txt | sed -n '1p' | awk '{print $2}'`
	if [ "$LOGIN_STATE" == "302" ] ;then
		WEIBO_GSID=`cat login_state_header.txt | grep Location | sed 's/%26/\n/g;s/%3D/\n/g' | sed -n '2p'`
		echo  ${WEIBO_GSID} > ${WEISH_CFG}
	elif [ "$LOGIN_STATE" == "200" ]; then
		rm login_state_header.txt
		echo "帐号或者密码错误!"
		exit
	fi
	rm login_state_header.txt
...
function load_weibo()
{
	...
	${CURL} $WEIBO_URL$WEIBO_GSID"&page=$1" 2>/dev/null  | sed 's/>/>\n/g' | sed '1,102d'| sed ':a;N;s/\n/\t/;ba;'|sed 's/<div class=\"c\"/\r\n\r\n<div class=\"c\"/g' | sed -e :a -e 's/<[^>]*>//g;/</N;//ba' |  sed 's/[[:space:]]//g' | sed  's/&nbsp;//g'  | sed '$d' | sed '$d' | sed '$d' | sed 's/首页.反馈.帮助.举报.退出/   [WEISH] /g'
}
